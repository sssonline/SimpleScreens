    ...
    <fo:page-sequence master-reference="letter-portrait" id="mainSequence">
        <fo:static-content flow-name="xsl-region-before">
            <fo:block font-size="14pt" text-align="center"
                      margin-bottom="0.1in"><#if firstPartInfo.isCustomerInternalOrg>Purchase<#else>Sales</#if> Order
                #${orderId}</fo:block>
                ...
                <fo:table-body>
                    <fo:table-row>
                        ...
                        <fo:table-cell padding="3pt" width="2in">
                            ...
                            <fo:block><#if orderHeader.placedDate??>${ec.l10n.format(orderHeader.placedDate, dateFormat)}<#else>Not
                                    yet placed</#if></fo:block>
        ...
        <fo:static-content flow-name="xsl-region-after" font-size="8pt">
            <fo:block border-top="thin solid black">
                ...
                <fo:block text-align="center">Order #${orderId}
                    -- <#if orderHeader.placedDate??>${ec.l10n.format(orderHeader.placedDate, "dd MMM yyyy")}<#else>Not
                        yet placed</#if> -- Page
                    <fo:page-number/>
                </fo:block>
            </fo:block>
            ...
            <#list orderPartInfoList as orderPartInfo>
                    ...
                    <fo:table-body>
                        ...
                        <fo:table-cell padding="3pt" width="2in">
                            ...
                            <#if orderPart.otherPartyOrderId?has_content>
                                <fo:block
                                        font-weight="bold"><#if orderPartInfo.isVendorInternalOrg>Customer<#else>Vendor</#if>
                                    Order #
                                </fo:block>
                        ...
                        <fo:table-cell padding="3pt" font-size="10pt" width="3.5in">
                            ...
                            <#if contactInfo.postalAddress.toName?has_content>
                                <fo:block font-weight="bold">To: <@encodeText contactInfo.postalAddress.toName/></fo:block>
                            </#if>
                            <#if contactInfo.postalAddress.attnName?has_content>
                                <fo:block font-weight="bold">
                                    Attn: <@encodeText contactInfo.postalAddress.attnName/></fo:block>
                            </#if>
                                <#else>
                                    <fo:block
                                            font-weight="bold"><@encodeText orderPartInfo.customerDetail.organizationName!""/> <@encodeText (orderPartInfo.customerDetail.firstName)!""/> <@encodeText (orderPartInfo.customerDetail.middleName)!""/> <@encodeText (orderPartInfo.customerDetail.lastName)!""/></fo:block>
                                </#if>
                                <fo:block><@encodeText (contactInfo.postalAddress.address1)!""/><#if contactInfo.postalAddress.unitNumber?has_content>
                                        #<@encodeText contactInfo.postalAddress.unitNumber/></#if></fo:block>
                            <#if contactInfo.postalAddress.address2?has_content>
                                <fo:block><@encodeText contactInfo.postalAddress.address2/></fo:block>
                            </#if>
                ...
                <fo:table table-layout="fixed" width="100%">
                    <fo:table-header font-size="9pt" border-bottom="solid black">
                        <fo:table-cell width="0.3in" padding="${cellPadding}">
                            <fo:block text-align="center">Item</fo:block>
                        </fo:table-cell>
                    <#if !showUpc>
                        <fo:table-cell width="1in" padding="${cellPadding}">
                            <fo:block>Type</fo:block>
                        </fo:table-cell>
                    </#if>
                        <fo:table-cell width="2.8in" padding="${cellPadding}">
                            <fo:block>Description</fo:block>
                        </fo:table-cell>
                    <#if showUpc>
                        <fo:table-cell width="1in" padding="${cellPadding}">
                            <fo:block>UPC</fo:block>
                        </fo:table-cell>
                    </#if>
                        <fo:table-cell width="0.8in" padding="${cellPadding}">
                            <fo:block>Required By</fo:block>
                        </fo:table-cell>
                        <fo:table-cell width="0.6in" padding="${cellPadding}">
                            <fo:block text-align="center">Qty</fo:block>
                        </fo:table-cell>
                        <fo:table-cell width="0.9in" padding="${cellPadding}">
                    ...
                    <fo:table-body>
                        <#list orderPartInfo.partOrderItemList as orderItem>
                            ...
                            <fo:table-row font-size="8pt"
                                          border-bottom="thin solid black"<#if !orderItem.parentItemSeqId?has_content>
                                font-weight="bold"</#if>>
                                <fo:table-cell padding="${cellPadding}">
                                    <fo:block
                                            text-align="<#if !orderItem.parentItemSeqId?has_content>center<#else>right</#if>">${orderItem.orderItemSeqId}</fo:block>
                                </fo:table-cell>
                        <#if !showUpc>
                            <fo:table-cell padding="${cellPadding}">
                                <fo:block><@encodeText (itemTypeEnum.description)!""/></fo:block>
                            </fo:table-cell>
                        </#if>
                                <fo:table-cell padding="${cellPadding}">
                        ...
                        <#if showUpc>
                            <fo:table-cell padding="${cellPadding}">
                                ...
                                <fo:table-cell padding="${cellPadding}">
                                    <fo:block>${ec.l10n.format(orderItem.requiredByDate, dateFormat)}</fo:block>
                                </fo:table-cell>
                                <fo:table-cell padding="${cellPadding}">
                                    <fo:block text-align="center">${orderItem.quantity!"1"}</fo:block>
                                </fo:table-cell>
                                <fo:table-cell padding="${cellPadding}">
                                    <fo:block
                                            text-align="right">${ec.l10n.formatCurrency(orderItem.unitAmount!0, orderHeader.currencyUomId, 2)}</fo:block>
                                </fo:table-cell>
                                <fo:table-cell padding="${cellPadding}">
                                    <fo:block
                                            text-align="right">${ec.l10n.formatCurrency(orderItemTotalOut.itemTotal, orderHeader.currencyUomId, 2)}</fo:block>
                                </fo:table-cell>
                            </fo:table-row>
                        </#list>
                        <fo:table-row font-size="9pt" border-top="solid black">
                            ...
                            <fo:table-cell padding="${cellPadding}">
                                <fo:block
                                        text-align="right">${ec.l10n.formatCurrency(orderPart.partTotal, orderHeader.currencyUomId)}</fo:block>
                            </fo:table-cell>
                        </fo:table-row>
                    </fo:table-body>
                </fo:table>
                ...
                <#-- <fo:block margin-top="0.2in" font-weight="bold">Shipping Instructions</fo:block> -->
                    <fo:block margin-top="0.2in"
                              linefeed-treatment="preserve"><@encodeText orderPartInfo.orderPart.shippingInstructions/></fo:block>
                </#if>
